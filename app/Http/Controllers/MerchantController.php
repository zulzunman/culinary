...
class MerchantController extends Controller
{
    ...
    public function updateAfterRegist()
    {
        $user = Auth::user()->id;
        $merchantProfile = MerchantProfile::where('user_id', $user)->first();
        $religions = Religion::all(); // Add this line
        ...
        // Mengirim data ke view
        return view('merchant.editAfterRegist', compact('user', 'merchantProfile', 'religions', 'cities'));
    }

    public function editAfterRegist(Request $request)
    {
        // Find user by ID
        $user = Auth::user();
        $merchantProfile = MerchantProfile::where('user_id', $user->id)->first();
        ...
        try {
            ...
            DB::commit(); // Jika semua proses berhasil
            return redirect()->route('store.edit-after-regist')->with('success', 'Berhasil melengkapi data');
        } catch (\Exception $e) {
            DB::rollBack(); // Membatalkan semua perubahan jika terjadi error
            return back()->withErrors(['error' => 'Gagal menyimpan data: ' . $e->getMessage()]);
        }
